CREATE OR REPLACE PACKAGE student_package AS
    PROCEDURE insert_student(s_id in number, s_name in varchar2, s_email in varchar2, s_phone_number in number, did in number,
                             a_year in number);
...
CREATE OR REPLACE PACKAGE BODY student_package AS
    ...
    BEGIN
        ...
        elsif check_student_id(s_id) THEN
            raise_application_error(-20002,'student id already exists');

        elsif not check_email_validity(s_email) THEN
            raise_application_error(-20001, 'email already exists');

        end if;
        insert into STUDENT(id, name, email, phone, dept_id, admitted_year) values (s_id, s_name, s_email, s_phone_number, did, a_year);
    END;
